  ...
  init() {
    ...
    if (!mobileTablet) {
      ...
      this.mouseFunction = MagnetMouse.throttle((e) => {
        ...
        if (this.elementFollow.length > 0) {
          for (let i = 0; i < this.elementFollow.length; i++) {
            this.elementFollow[i].style.transform = 'translate3d(' + (e.x - (this.elementFollow[i].offsetWidth / 2 + window.pageXOffset)) + 'px,' + (e.y - (this.elementFollow[i].offsetHeight / 2 + window.pageYOffset)) + 'px, 0)';
          }
        }
